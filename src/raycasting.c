...
void render_vertical_line(struct FrameBuffer *frame, int x, int y_start,
                          int y_end, struct Color color) {
...
void render(struct FrameBuffer *frame, struct ImageBuffer *color_map,
            struct ImageBuffer *height_map, struct Camera *camera) {
  ...
  for (float z = 1; z < camera->distance; z += delta_z) {
    ...
    float point_right_y = (-cosphi * z - sinphi * z) + camera->position[2];
    ...
    for (int x = frame->clip_left_x; x < frame->clip_right_x; x++) {
      ...
      int terrain_height =
          get_image_grey(height_map, point_left_x, point_left_y);
      ...
      if (height_on_screen < y_start) {
        struct Color color =
            get_image_color(color_map, point_left_x, point_left_y);
